...
export function HeroFeaturedToggle({ postId, isHero, isFeatured, featuredOrder, heroPostId, leftPostId, rightPostId, part }: Props) {
  ...
  if (part === 'hero') {
    ...
    return (
      <input
        ...
        className={`rounded border-gray-300 text-gray-900 focus:ring-0 ${heroTaken ? 'opacity-25 cursor-not-allowed' : 'cursor-pointer'}`}
      />
    )
  }
    ...
    <div className="flex items-center gap-1">
      <button
        ...
        className={`px-2 py-0.5 text-[10px] font-medium rounded transition-colors ${
          leftActive ? 'bg-blue-500 text-white' :
          leftTaken ? 'bg-gray-100 text-gray-300 cursor-not-allowed' :
      ...
      <button
        ...
        className={`px-2 py-0.5 text-[10px] font-medium rounded transition-colors ${
          rightActive ? 'bg-blue-500 text-white' :
          rightTaken ? 'bg-gray-100 text-gray-300 cursor-not-allowed' :
          'bg-gray-100 text-gray-500 hover:bg-blue-50 hover:text-blue-600'
        }`}
      >Right</button>
    </div>
